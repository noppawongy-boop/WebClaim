...
/* Combined exporter: one claim per row with every image anchored in columns on that same row. */
(() => {
  const button=document.querySelector('#excelCombinedBtn');
  if(!button)return;
  const collect=value=>{const out=[];const walk=v=>{if(!v)return;if(Array.isArray(v))return v.forEach(walk);if(typeof v==='string'&&v.startsWith('data:image/'))out.push(v);else if(typeof v==='object')walk(v.image||v.src||v.data||v.url)};walk(value);return out};
  button.onclick=()=>{
    const columns=[['วันที่ติดต่อ','contactDate'],['วันที่ซื้อ','purchaseDate'],['ช่องทาง','purchaseChannel'],['รหัสสาขา','branchCode'],['ชื่อสาขา','branchName'],['ชื่อลูกค้า','customerName'],['ประเภทเคลม','claimType'],['สินค้า / รุ่น','product'],['SKU','productSku'],['เลขออเดอร์','orderNumber'],['รายละเอียด','notes'],['รุ่นที่เปลี่ยน','resolvedModel'],['SKU ที่เปลี่ยน','resolvedSku'],['วันที่ส่งคืน','returnDate'],['ขนส่ง','courier'],['Tracking','trackingNumber'],['วิธีการจัดส่งสินค้า','deliveryMethod'],['ผลดำเนินการ','resolutionNotes']];
    const prepared=claims.map(record=>({record,images:[...collect(record.receipt),...collect(record.problemImage),...collect(record.problems)]}));
    const maxImages=Math.max(0,...prepared.map(x=>x.images.length));
    const headers=columns.map(x=>x[0]).concat(Array.from({length:maxImages},(_,i)=>`รูปภาพ ${i+1}`));
    const media=[],anchors=[],relationships=[];
    prepared.forEach((item,rowIndex)=>item.images.forEach((data,imageIndex)=>{
      const info=imageInfo(data,`combined${media.length+1}`);if(!info)return;
      const ext=info.name.split('.').pop(),file=`combined${media.length+1}.${ext}`,relId=media.length+1;
      media.push({name:`xl/media/${file}`,data:info.bytes});relationships.push(`<Relationship Id="rId${relId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/${file}"/>`);
      anchors.push(`<xdr:oneCellAnchor><xdr:from><xdr:col>${columns.length+imageIndex}</xdr:col><xdr:colOff>50000</xdr:colOff><xdr:row>${rowIndex+1}</xdr:row><xdr:rowOff>50000</xdr:rowOff></xdr:from><xdr:ext cx="1200000" cy="900000"/><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="${relId+1}" name="${file}"/><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId${relId}"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:oneCellAnchor>`);
    }));
    const widths=headers.map((_,i)=>`<col min="${i+1}" max="${i+1}" width="${i>=columns.length?18:i<3?15:i<10?22:28}" customWidth="1"/>`).join('');
    const headerRow=`<row r="1" ht="24" customHeight="1">${headers.map((v,i)=>xCell(v,i,1,1)).join('')}</row>`;
    const dataRows=prepared.map((item,index)=>`<row r="${index+2}" ht="85" customHeight="1">${columns.map((c,i)=>xCell(item.record[c[1]]??'',i,index+2)).join('')}</row>`).join('');
    const sheet=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${widths}</cols><sheetData>${headerRow}${dataRows}</sheetData><autoFilter ref="A1:${excelCol(headers.length-1)}${Math.max(1,prepared.length+1)}"/>${media.length?'<drawing r:id="rId1"/>':''}</worksheet>`;
    const styles='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="10"/><name val="Tahoma"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Tahoma"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF17262D"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>';
    const types=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="png" ContentType="image/png"/><Default Extension="jpg" ContentType="image/jpeg"/><Default Extension="gif" ContentType="image/gif"/><Default Extension="webp" ContentType="image/webp"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/>${media.length?'<Override PartName="/xl/drawings/drawing1.xml" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>':''}</Types>`;
    const files=[{name:'[Content_Types].xml',data:types},{name:'_rels/.rels',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>'},{name:'xl/workbook.xml',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="ข้อมูลรวม" sheetId="1" r:id="rId1"/></sheets></workbook>'},{name:'xl/_rels/workbook.xml.rels',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>'},{name:'xl/styles.xml',data:styles},{name:'xl/worksheets/sheet1.xml',data:sheet}];
    if(media.length){files.push({name:'xl/worksheets/_rels/sheet1.xml.rels',data:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing1.xml"/></Relationships>'},{name:'xl/drawings/drawing1.xml',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">${anchors.join('')}</xdr:wsDr>`},{name:'xl/drawings/_rels/drawing1.xml.rels',data:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${relationships.join('')}</Relationships>`},...media)}
    download(new Blob([makeZip(files)],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),`claim-log-รวมรูป-${new Date().toLocaleDateString('sv-SE')}.xlsx`);toast(`ดึงรวม ${prepared.length} รายการ ${media.length} รูป`)
  };
})();
